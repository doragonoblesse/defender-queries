## Solorigate Queries (2020)

```markdown
//detects events when SolarWinds processes attempted to launch the cmd.exe command prompt using the echo command
DeviceProcessEvents
| where InitiatingProcessFileName =~ "SolarWinds.BusinessLayerHost.exe"
| where FileName == "cmd.exe" and ProcessCommandLine has "echo"
//

//query retrieves an inventory of SolarWinds Orion software use
DeviceTvmSoftwareInventoryVulnerabilities
| where SoftwareVendor == 'solarwinds'
| where SoftwareName startswith 'orion'
| summarize dcount(DeviceName) by SoftwareName, DeviceName
| sort by dcount_DeviceName desc
//

//detects events when SolarWinds processes launched PowerShell commands that were possibly encoded in Base64
DeviceProcessEvents
| where InitiatingProcessFileName =~ "SolarWinds.BusinessLayerHost.exe"
...
//query searches Threat and Vulnerability Management (TVM) data for Orion software known to be affected by Solorigate
DeviceTvmSoftwareInventoryVulnerabilities
...
//query detects network connection events involving DNS lookups associated with the Solorigate threat actor's command-and-control infrastructure
DeviceNetworkEvents
...
//Search for the files that are using the compromised certificate. You can remove the comments to
...
//query Solarwinds using IOCs
let SHA1srch = dynamic([
...
"3faf98a6dfa357e3dcd221fa09039464",
...
"v1ktd644g9uen95a00h29jv2fn34tkr3.appsync-api.eu-west-1.avsvmcloud.com",
"urle4tgvrv5lln28ine0covirsvu1.appsync-api.eu-west-1.avsvmcloud.com",
"3ae2cisaj9o2vm5n4cfjbqf.appsync-api.eu-west-1.avsvmcloud.com",
"8cngei63kcpgho7kern0le2ve2sn0te2.appsync-api.eu-west-1.avsvmcloud.com",
"sm08vjhrqlenk7i72d60i1ucl026rswo.appsync-api.eu-west-1.avsvmcloud.com",
"2bdo5s70oc51vu3de3bvrq60eiw.appsync-api.eu-west-1.avsvmcloud.com",
"qcalp99pnc2l0344ovirv6owr0iovi.appsync-api.eu-west-1.avsvmcloud.com",
"jbq3rh7rjdghmmcxco0ge2sd.appsync-api.eu-west-1.avsvmcloud.com",
"rhkaf7cekkeuvboi2fi8.appsync-api.eu-west-1.avsvmcloud.com",
"8tvp0990935eitt5hjvcbmv.appsync-api.eu-west-1.avsvmcloud.com",
"rhgrgek4rrc52ooiaig1.appsync-api.eu-west-1.avsvmcloud.com",
...
let EmailURL_srch = EmailEvents | join(EmailUrlInfo) on NetworkMessageId | where EmailDirection == "Inbound" | project NetworkMessageId, SenderFromAddress, SenderMailFromAddress, SenderIPv4, RecipientEmailAddress, Subject, DeliveryLocation, Url, Timestamp;
search in (DeviceNetworkEvents, DeviceEvents, DeviceProcessEvents, DeviceFileEvents, DeviceImageLoadEvents, EmailAttachmentInfo, EmailURL_srch, EmailEvents, AppFileEvents, IdentityLogonEvents)
SHA1 in (SHA1srch) or SHA256 in (SHA256srch) or MD5 in (MD5srch) or InitiatingProcessSHA1 in (SHA1srch) or InitiatingProcessSHA256 in (SHA256srch) or InitiatingProcessMD5 in (MD5srch)
or IPAddress in (ipsrch) or RemoteIP in (ipsrch) or FileOriginIP in (ipsrch) or RequestSourceIP in (ipsrch) or DestinationIPAddress in (ipsrch) or SenderIPv4 in (ipsrch)
or RemoteUrl in (urlsrch) or FileOriginUrl in (urlsrch) or FileOriginReferrerUrl in (urlsrch)
or Url has_any (urlsrch) | limit 100
//
